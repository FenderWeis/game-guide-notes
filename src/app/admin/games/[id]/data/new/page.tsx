...
interface NewGameDataPageProps {
  params: { id: string }
}
...
export default function NewGameDataPage({ params }: NewGameDataPageProps) {
  ...
  useEffect(() => {
    ...
    const fetchGame = async () => {
      const { data } = await supabase.from('games').select('name').eq('id', params.id).single()
      setGame(data)
    }
    fetchGame()
  }, [supabase, params.id])
  ...
  const deleteContentBlock = (moduleIndex: number, blockIndex: number) => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    console.log('hasValidModule:', hasValidModule)
    if (!hasValidModule) {
    ...
    try {
      console.log('Step 1: Inserting game_data...')
      console.log('Params:', {
        game_id: params.id,
        title: title,
      ...
      const { data: gameData, error: insertError } = await supabase.from('game_data').insert({
        game_id: params.id,
        title,
        content: '',
        category_id: categoryId,
      ...
      if (insertError) {
        console.error('Insert game_data error:', {
      ...
      console.log('Step 1 done: game_data inserted with id:', gameData.id)
      ...
      for (let i = 0; i < modules.length; i++) {
        ...
        for (let j = 0; j < module.contentBlocks.length; j++) {
          ...
          const { error: blockError } = await supabase.from('game_data_content_blocks').insert({
            game_data_id: gameData.id,
